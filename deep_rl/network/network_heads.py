...
class CategoricalActorCriticNet_two(nn.Module, BaseNet):
    def __init__(self,state_dim, action_dim, phi_body_1, phi_body_2, actor_body=None, critic_body=None):
        super(CategoricalActorCriticNet_two,self).__init__()
        if actor_body is None: actor_body = DummyBody(phi_body_1.feature_dim)
        if critic_body is None: critic_body = DummyBody(phi_body_2.feature_dim)
        self.phi_body_1 = phi_body_1
        self.phi_body_2 = phi_body_2
        self.actor_body = actor_body
        self.critic_body = critic_body
        self.fc_action = layer_init(nn.Linear(actor_body.feature_dim, action_dim), 1e-3)
        self.fc_critic = layer_init(nn.Linear(critic_body.feature_dim, 1), 1e-3)
        ...
        self.actor_params = list(self.actor_body.parameters()) + list(self.fc_action.parameters())
        self.critic_params = list(self.critic_body.parameters()) + list(self.fc_critic.parameters())
        self.phi_params = list(self.phi_body_1.parameters()) + list(self.phi_body_2.parameters())

        self.to(Config.DEVICE)

    def forward(self, obs, action=None):
        obs = tensor(obs)
        phi_1 = self.phi_body_1(obs)
        phi_2 = self.phi_body_2(obs)
        phi_a = self.actor_body(phi_1)
        phi_v = self.critic_body(phi_2)
        logits = self.fc_action(phi_a)
        v = self.fc_critic(phi_v)
        dist = torch.distributions.Categorical(logits=logits)
        if action is None:
