...
class NatureConvBody(nn.Module):
    def __init__(self, in_channels=4, phi_dim = 512):
        super(NatureConvBody, self).__init__()
        self.feature_dim = phi_dim
...
class ConvBody(nn.Module):
    def __init__(self, in_channels=4, phi_dim = 512):
        ...
        self.conv1 = layer_init(nn.Conv2d(in_channels, 32, kernel_size=8, stride=4))
        self.conv2 = layer_init(nn.Conv2d(32, 64, kernel_size=4, stride=2))
        self.fc3 = layer_init(nn.Linear(9 * 9 * 64, self.feature_dim))
...
class ConvFCBody(nn.Module):
    def __init__(self, in_channels=4, phi_dim = 512):
        super(ConvFCBody,self).__init__()
        self.feature_dim = phi_dim
        self.conv1 = layer_init(nn.Conv2d(in_channels, 32, kernel_size=8, stride=4))
        self.conv2 = layer_init(nn.Conv2d(32, 64, kernel_size=4, stride=2))
        self.fc3 = layer_init(nn.Linear(9 * 9 * 64, self.feature_dim))
        self.fc4 = layer_init(nn.Linear(self.feature_dim, self.feature_dim))
...
class ConvFCBodyMaze(nn.Module):
    def __init__(self, size=20, phi_dim = 512, in_channels=3):
        super(ConvFCBodyMaze,self).__init__()
        self.feature_dim = phi_dim
        ...
        bsize = 5
        x = self.conv2(self.conv1(torch.randn(bsize, in_channels, size, size)))
        nhid = x.view(bsize, -1).shape[1]
        
        self.fc3 = layer_init(nn.Linear(nhid, self.feature_dim))
        self.fc4 = layer_init(nn.Linear(self.feature_dim, self.feature_dim))
